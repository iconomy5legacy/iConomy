...
public class Settings {
	...
	private static Path configPath = iConomy.getPlugin().getDataFolder().toPath().resolve("config.yml");
	...
	public static void loadConfig() {
		if (FileMgmt.checkOrCreateFile(configPath.toString())) {
			...
			setDefaults(iConomy.getPlugin().getDescription().getVersion(), configPath);
			config.save();
		}
	}
	...
	private static void sendError(String msg) {
		iConomy.getPlugin().getLogger().severe("Error could not read " + msg);
	}
